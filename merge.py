...
def merge_csv(input_file_1, input_file_2, output_file):
    ...
    df_1 = pd.read_csv(input_file_1, header=None, names=["KEY", "COLUMN_2"], encoding='utf-8')  # Wczytanie z nagłówkami "KEY" i "COLUMN_2"
    df_2 = pd.read_csv(input_file_2, header=None, names=["KEY", "COLUMN_3"], encoding='utf-8')
     
    # Łączenie dwóch DataFrame'ów po kolumnie "KEY"
    df_merged = pd.merge(df_1, df_2, on="KEY", how="left")  # Możesz zmienić 'how' na 'outer' w zależności od potrzeb
    ...
    df_merged.to_csv(output_file, header=False, index=False, encoding='utf-8-sig')
